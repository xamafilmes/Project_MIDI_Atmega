...
void sdCardSsLow(void);
void sdCardSsHigh(void);

spiSlaveDevice_s spiSlaveSdCard;

/* Peripheral controls (Platform dependent) */
#define CS_LOW()		spiSlaveDeviceSsLow(&spiSlaveSdCard)
#define CS_HIGH()		spiSlaveDeviceSsHigh(&spiSlaveSdCard)
...
#define	FCLK_SLOW()		SPCR |=  ((1<<SPR1) | (1<<SPR0))
#define	FCLK_FAST()		SPCR &= ~((1<<SPR1) | (1<<SPR0))

void sdCardCreate(pinLocation_e ss)
{
    spiSlaveSdCard.ssLocation = ss;
    spiSlaveSdCard.ddrOffset = 1 + (3 * (ss >> 4));
    spiSlaveSdCard.portOffset = 2 + (3 * (ss >> 4));
    spiSlaveSdCard.bitOffset = (ss & 0x07);

    setBit(_SFR_IO8(spiSlaveSdCard.portOffset), spiSlaveSdCard.bitOffset);
    setBit(_SFR_IO8(spiSlaveSdCard.ddrOffset), spiSlaveSdCard.bitOffset);
...
void sdCardSsLow(void)
{
    clrBit(_SFR_IO8(spiSlaveSdCard.portOffset), spiSlaveSdCard.bitOffset);
...
)
{
    ...
    case ISDIO_READ:
        ...
        if(send_cmd(CMD48, 0x80000000 | (DWORD)sdi->func << 28 | (DWORD)sdi->addr << 9 | ((sdi->ndata - 1) & 0x1FF)) == 0) {
            ...
            if(rc == 0xFE) {
                for(bp = (BYTE *)(sdi->data), dc = sdi->ndata; dc; dc--) {
                    *bp++ = xchg_spi(0xFF);
                }
